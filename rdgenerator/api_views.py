...
from .views import generate_custom_client, _get_run_status, public_gen_url, public_gen_url, public_gen_url
...
def validate_generate_params(data):
    ...
    # Use the live generator version choices, not an upstream hard-coded release.
    version_form = GenerateForm()
    version_choices = [str(choice[0]) for choice in version_form.fields['version'].choices]
    default_version = str(version_form.fields['version'].initial or (version_choices[0] if version_choices else 'master'))
